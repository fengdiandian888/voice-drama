...
处理：切句(逗号合并+去残留标点) + 上位词说话人识别 + 启发式语气 + 关键词题材标签
      + 敏感标记 + 统计式篇级字段 + 段内时间戳插值 + 同音错别字规则替换。
title_zh 留空（需读原文翻译，不臆造）；篇级总结为统计式并标注 (自动)。

本轮改进（对照调整清单）：
  A3 时间戳插值：切句后在 [段首ts, 下一段ts) 间线性插值，解决"全堆0秒"。
  A4/B4 切句策略：逗号不再切碎（逗号本为句内停顿），只按句末标点/空格切；句尾去残留标点；全角统一。
  C8 标点统一：全角逗号/句号统一。
  A1(规则版) 错别字：THES 同音近音词典替换（覆盖高频转写错，待 LLM 精校补余）。
"""
...
# ── A1(规则版) 同音/近音错别字词典 ──
# 仅收录高置信误识（语音转写把 A 听成 B）。可在此扩展。
# 注意："管娇/调教/影帝"等是本频道术语，非错别字，勿改。
THES = {
    "坐溪": "作息",
    "坐夕": "作息",
    "做习": "作息",
    "坐夕惯": "作息惯",
    "低血人轻": "低血轻",   # 高跟鞋语境，原指"低血轻"(轻量/低跟)，保留待精校
    "在次": "再次",
    "那在": "在那",
    "那各": "那个",
    "哪各": "那个",
...
_THES_LONG = {k: v for k, v in THES.items() if len(k) >= 2}  # 长词优先，避免单字误替
_THES_SORTED = sorted(_THES_LONG.keys(), key=len, reverse=True)
...
def fix_typo(text):
    """A1(规则版)：同音近音错别字替换。仅长词优先，避免短词误替。"""
    t = text
    for k in _THES_SORTED:
        if k in t:
            t = t.replace(k, THES[k])
    return t


def split_sentences(text):
    """A4/B4/C8：逗号不切碎(逗号本为句内停顿)，只按句末标点/空格切；句尾去残留标点；全角统一。"""
    t = text
    # 全角统一
    t = t.replace(",", "，").replace("、", "，")
    t = t.replace(".", "。").replace("!", "！").replace("?", "？")
    # 按 句末标点 或 空白 切；逗号保留为句内停顿
    raw = re.split(r"[\s。！？;；]+", t)
    out = []
    for s in raw:
...
def interp_ts(ts, next_ts, j, k):
    """A3 时间戳插值：在 [段首ts, 下一段ts) 间按句序等分；无下一段或无法解析则回退段首ts。"""
    a = _to_sec(ts)
    if a is None:
        return ts
    b = _to_sec(next_ts) if next_ts else None
    if b is None or b <= a:
        return ts
    frac = (j / k) if k > 1 else 0.0
    return _fmt_sec(a + (b - a) * frac)
...
def enrich_video(v):
    ...
    lines = []
    tones = []
    segs = [(str(x[0]), str(x[1])) for x in lines_src]
    n = len(segs)
    for i, (ts, text) in enumerate(segs):
        next_ts = segs[i + 1][0] if i + 1 < n else None
        sents = split_sentences(text)
        k = len(sents)
        for j, sent in enumerate(sents):
            sent = fix_typo(sent)  # A1 错别字规则替换（显示文本）
            tone = guess_tone(sent)
            tones.append(tone)
            tstamp = interp_ts(ts, next_ts, j, k)  # A3 插值
            lines.append({"ts": tstamp, "text": sent, "speaker": speaker,
                          "tone": tone, "sound": []})
