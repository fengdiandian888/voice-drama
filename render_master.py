...
<style>
  ...
  *{box-sizing:border-box}
  ...
  .dline{margin:0;padding:7px 0 7px 14px;border-left:3px solid var(--line);line-height:1.85}
  .dline .spk{display:block;font-size:calc(13px * var(--rs,1));font-weight:800;margin-bottom:3px;letter-spacing:.02em}
  .dline .dia{font-size:calc(16px * var(--rs,1));line-height:1.95;color:#0f172a;text-align:justify}
  ...
  body.edit .line-speaker{cursor:pointer;outline:1px dashed #cbd5e1;outline-offset:2px}
  ...
  @media(max-width:780px){.filters .legend{display:none}.hbtn.txtonly{display:none}}
</style>
</head>
<body>
<header>
...
function detailLineHTML(r,i,l,speaker,tone,hier,show,kw){
  const tc=toneColor(tone);
  const neutral=tone&&NEUTRAL_TONE.test(tone);
  const bar=(tc&&!neutral)?' style="border-left-color:'+tc+'"':'';
  const spkAttr='data-vid="'+esc(r.id)+'" data-li="'+i+'"';
  const spk=show?'<span class="spk line-speaker" data-hier="'+hier+'" '+spkAttr+'>'+hl(speaker,kw)+'</span>':'';
  const text=fmtLine(l.text,kw);
  const snd=(l.sound||[]).map(s=>'<span class="snd">'+hl(s,kw)+'</span>').join(" ");
  return '<div class="dline">'+spk+'<div class="dia"'+bar+'>'+text+(snd?' '+snd:'')+'</div></div>';
}
...
function renderDetail(r){
  ...
  const lines=(r.lines||[]).map((l,i)=>{
    ...
    const show=speaker&&speaker!==prev;
    prev=speaker||prev;
    return detailLineHTML(r,i,l,speaker,tone,hier,show,state.kw);
  }).join("") || '<div class="empty">（无台词）</div>';
...
document.addEventListener("click",e=>{
  if(!EDIT) return;
  const t=e.target.closest(".line-speaker");
  if(!t) return;
