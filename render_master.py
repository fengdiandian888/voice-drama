...
<style>
  ...
  *{box-sizing:border-box}
  ...
  /* ── 数据总览面板（题材/角色/强度，可点击筛选）── */
  .dashcols{display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:18px;max-width:1280px;margin:0 auto}
  .dashh{font-size:12px;font-weight:700;color:var(--accent);margin-bottom:6px;letter-spacing:.04em}
  .dashtags{display:flex;flex-wrap:wrap;gap:6px;align-items:flex-start}
  .dtag,.dspk{cursor:pointer;border:1px solid #e2e8f0;background:#fff;color:#475569;border-radius:999px;padding:3px 10px;font-size:12px;line-height:1.5}
  .dtag b,.dspk b{color:#0f172a}
  .dtag:hover,.dspk:hover{border-color:#2563eb;color:#2563eb}
  .dtag.on,.dspk.on{background:#2563eb;color:#fff;border-color:#2563eb}
  .dtag.on b,.dspk.on b{color:#fff}
  ...
  .grp{margin:18px 0 8px;padding:6px 12px;background:linear-gradient(90deg,#eff6ff,#f5f3ff);border-left:4px solid var(--accent);border-radius:8px;font-weight:700;font-size:14px;color:#1e3a8a;display:flex;align-items:center;gap:8px}
  .grp-c{font-size:11px;font-weight:600;color:#64748b;background:#fff;border:1px solid #dbeafe;border-radius:10px;padding:1px 8px}
  .sortbtns{margin-left:6px}
  .sortbtns b{margin-right:4px}
  .sortbtns button{font-size:12px;padding:3px 10px;border:1px solid var(--line);background:#fff;border-radius:14px;cursor:pointer;color:#475569}
...
<header>
  ...
  <div class="legend">
    ...
    <span class="sortbtns" id="sortBtns">
      <b>排序：</b>
      <button data-sort="genre" class="on">题材分组</button>
      <button data-sort="orig">原序</button>
      <button data-sort="dur">时长↑</button>
      <button data-sort="intensity">强度↓</button>
    </span>
    <button class="editbtn" id="editBtn">✎ 编辑台词</button>
    <button class="resetbtn" id="ovReset">清空纠错</button>
    <button class="editbtn" id="dashBtn">数据总览</button>
  </div>
</header>
<div id="dash" style="display:none;background:var(--panel);border-bottom:1px solid var(--line);padding:14px 20px">
  <div class="dashcols">
    ...
    <div><div class="dashh">强度分布（点击筛选）</div><div id="dashIntensity" class="dashtags"></div>
      <div id="dashHier" style="margin-top:10px;font-size:12px;color:#64748b"></div></div>
  </div>
...
/* ===== 排序：题材分组 / 原序 / 时长 / 强度 ===== */
let SORT_MODE = "genre";
function groupKey(r){ return ((r.genre_tags&&r.genre_tags[0])||"未分类"); }
function parseDur(d){
  d=(d||"").toString().trim();
...
function applySort(list){
  ...
  if(SORT_MODE==="dur"){ arr.sort((a,b)=>parseDur(a.duration)-parseDur(b.duration)); return arr; }
  if(SORT_MODE==="intensity"){ const o={"高":0,"中":1,"低":2}; arr.sort((a,b)=>(o[a.intensity]??9)-(o[b.intensity]??9)); return arr; }
  if(SORT_MODE==="genre"){
    const freq={}; DATA.forEach(r=>{ const k=groupKey(r); freq[k]=(freq[k]||0)+1; });
    arr.sort((a,b)=>{ const ka=groupKey(a),kb=groupKey(b); if(ka!==kb) return (freq[kb]||0)-(freq[ka]||0); return (a.no||0)-(b.no||0); });
    return arr;
  }
  arr.sort((a,b)=>(a.no||0)-(b.no||0)); return arr;
}
...
function render(list, kw){
  ...
  let html="", curG=null, counts={};
  if(SORT_MODE==="genre"){ list.forEach(r=>{ const g=groupKey(r); counts[g]=(counts[g]||0)+1; }); }
  list.forEach(r=>{
    if(SORT_MODE==="genre"){ const g=groupKey(r); if(g!==curG){ curG=g; html+='<div class="grp">'+esc(g)+' <span class="grp-c">'+counts[g]+'</span></div>'; } }
    html += cardHTML(r, kw);
  });
...
const FILTER = { hier:null, tone:null, genre:null, sensitive:null, speaker:null, intensity:null };
function filterHit(r){
  ...
  if(FILTER.speaker){ if(!(r.lines||[]).some((l,i)=> spkKey(r,i)===FILTER.speaker)) return false; }
  if(FILTER.intensity){ if(r.intensity!==FILTER.intensity) return false; }
  return true;
}
function search(kw){
...
function renderFilters(){
  ...
  const defs = FILTER_DEFS.map(f => (f.dim==="genre" && typeof TOP_GENRES!=="undefined" && TOP_GENRES.length)
    ? Object.assign({}, f, {opts: TOP_GENRES.slice(0,10).map(([t])=>[t,t])}) : f);
...
/* ===== 全局聚合：题材 / 角色 / 强度 / 身份 ===== */
function spkKey(r, li){ const ov=getOV(r.id,li); return (ov&&ov.speaker!==undefined)?ov.speaker:(r.lines[li].speaker||""); }
function spkHier(r, li){ const ov=getOV(r.id,li); const s=(ov&&ov.speaker!==undefined)?ov.speaker:(r.lines[li].speaker||""); return (ov&&ov.hier)?ov.hier:hierOf(s); }

const GENRE_COUNT={}, SPEAKER_COUNT={}, INTENSITY_COUNT={};
const HIER_TOTAL={sup:0,sub:0,other:0,unknown:0};
DATA.forEach(r=>{
  (r.genre_tags||[]).forEach(t=>{ GENRE_COUNT[t]=(GENRE_COUNT[t]||0)+1; });
  if(r.intensity) INTENSITY_COUNT[r.intensity]=(INTENSITY_COUNT[r.intensity]||0)+1;
  (r.lines||[]).forEach((l,i)=>{ const s=spkKey(r,i); if(s){ SPEAKER_COUNT[s]=(SPEAKER_COUNT[s]||0)+1; HIER_TOTAL[spkHier(r,i)]++; } });
});
...
function renderDash(){
  ...
  const gHtml=TOP_GENRES.slice(0,24).map(([t,c])=>{
    ...
    return '<button class="dtag" data-kind="genre" data-val="'+esc(t)+'" style="font-size:'+fs+'px">'+esc(t)+' <b>'+c+'</b></button>';
  }).join('');
  const sHtml=TOP_SPEAKERS.map(([s,c])=>{
    const h=HIER[s]?(HIER[s]==="sup"?"sup":HIER[s]==="sub"?"sub":"other"):"other";
    const color=h==="sup"?"#2563eb":h==="sub"?"#db2777":"#4b5563";
  ...
  const iHtml=Object.keys(INTENSITY_COUNT).sort().map(k=>{
    ...
    const color=k.indexOf("高")>=0?"#dc2626":k.indexOf("中")>=0?"#ea580c":"#0d9488";
    return '<button class="dtag" data-kind="intensity" data-val="'+esc(k)+'" style="border-color:'+color+';color:'+color+'">'+esc(k)+' <b>'+c+'</b></button>';
  }).join('');
  const hT=HIER_TOTAL;
  ...
  document.getElementById("dashHier").innerHTML='全库身份格局：上位 <b style="color:#2563eb">'+hT.sup+'</b> · 下位 <b style="color:#db2777">'+hT.sub+'</b> · 其他 <b style="color:#4b5563">'+hT.other+'</b> · 未识别 '+hT.unknown;
}
renderDash();
...
document.getElementById("sortBtns").addEventListener("click", e=>{
  ...
  SORT_MODE = b.dataset.sort;
  document.querySelectorAll("#sortBtns button[data-sort]").forEach(x=>x.classList.toggle("on", x.dataset.sort===SORT_MODE));
  search(document.getElementById("search").value);
});
...
document.getElementById("dash").addEventListener("click",e=>{
  ...
  document.querySelectorAll('#dash [data-kind="'+kind+'"]').forEach(x=>x.classList.toggle("on", x.dataset.val===FILTER[kind]));
  if(kind==="genre"){
    document.querySelectorAll('#filters .chip[data-dim="genre"]').forEach(c=>c.classList.toggle("on", c.dataset.val===FILTER.genre));
  }
  search(document.getElementById("search").value);
});
document.getElementById("dashClear").addEventListener("click",()=>{
  FILTER.speaker=null; FILTER.intensity=null; FILTER.genre=null;
  document.querySelectorAll('#dash [data-kind]').forEach(x=>x.classList.remove("on"));
  document.querySelectorAll('#filters .chip[data-dim="genre"]').forEach(c=>c.classList.toggle("on", c.dataset.val===""));
  search(document.getElementById("search").value);
});
