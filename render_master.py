...
function pieceSpeakers(r){
  const m={};
  (r.lines||[]).forEach((l,i)=>{
    const s=spkKey(r,i); if(!s) return;
    const h=spkHier(r,i);
    if(!m[s]) m[s]={name:s,hier:h,count:0};
    m[s].count++;
  });
  return Object.values(m).sort((a,b)=>b.count-a.count);
}
function charPanelHTML(r, kw){
  if(!r.enriched) return '';
  const sps=pieceSpeakers(r); if(!sps.length) return '';
  const max=sps[0].count;
  const rows=sps.map(s=>{
    const color=s.hier==="sup"?"#2563eb":s.hier==="sub"?"#db2777":s.hier==="other"?"#4b5563":"#9ca3af";
    const w=Math.max(6,Math.round(s.count/max*100));
    return '<div style="display:flex;align-items:center;gap:8px;margin:5px 0;font-size:12.5px">'+
      '<span style="flex:0 0 92px;color:'+color+';font-weight:600;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">'+hl(s.name,kw)+'</span>'+
      '<span style="flex:1;height:8px;background:#eef2f7;border-radius:5px;overflow:hidden"><i style="display:block;height:100%;width:'+w+'%;background:'+color+'"></i></span>'+
      '<span style="flex:0 0 26px;text-align:right;color:#64748b">'+s.count+'</span></div>';
  }).join('');
  const supN=sps.filter(s=>s.hier==="sup").length, subN=sps.filter(s=>s.hier==="sub").length;
  const pat=(supN||subN)?'<span style="font-size:11px;color:#64748b;margin-left:8px">格局：'+supN+' 上位 · '+subN+' 下位</span>':'';
  return '<div class="sec"><div class="t">角色关系'+pat+'</div>'+rows+'</div>';
}
